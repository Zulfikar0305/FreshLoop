// screens/npo_user/NPONotificationsScreen.tsx
...
type FilterType = 'All' | 'Unread' | 'Pickups' | 'Handoffs' | 'System';
const FILTERS: FilterType[] = ['All', 'Unread', 'Pickups', 'Handoffs', 'System'];

const NPO_META: Record<string, {
  icon: React.ComponentProps<typeof Feather>['name'];
  color: string;
  bg: string;
}> = {
  claim:    { icon: 'truck',           color: '#2D6A4F', bg: 'rgba(45,106,79,0.1)'   },
  donation: { icon: 'package',         color: '#2E7BBF', bg: 'rgba(46,123,191,0.1)'  },
...
function getMeta(type: string) {
  return NPO_META[type] ?? NPO_META.system;
}
...
function passesFilter(n: InAppNotification, filter: FilterType): boolean {
  switch (filter) {
    ...
    case 'Pickups':  return n.type === 'claim';
    case 'Handoffs': return n.type === 'donation';
      ...
      >
        ...
        <View style={{
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <Text style={{ color: '#1E293B', fontSize: 22, fontWeight: '800', letterSpacing: -0.5 }}>
              Notifications
            </Text>
            {unreadCount > 0 && (
          ...
          {unreadCount > 0 && (
            <TouchableOpacity onPress={markAllRead} activeOpacity={0.7}>
              <Text style={{ fontSize: 12, color: '#2D6A4F', fontWeight: '700' }}>Mark all read</Text>
            </TouchableOpacity>
          )}
        </View>

        <Text style={{ color: '#64748B', fontSize: 13, marginBottom: 20 }}>
          Stay on top of pickups, handoffs, and account updates.
        </Text>
        ...
        >
          {FILTERS.map(f => {
            ...
            return (
              ...
              >
                <Text style={{ fontSize: 13, fontWeight: '700', color: isActive ? '#fff' : '#94A3B8' }}>
                  {f}
                </Text>
              </TouchableOpacity>
            );
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
                  <View style={{
                    ...
                    backgroundColor: s.bg, alignItems: 'center', justifyContent: 'center', marginTop: 1,
                  }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                      <Text style={{ fontWeight: n.isRead ? '600' : '800', fontSize: 14, color: '#1E293B', flex: 1 }}>
                        {n.title}
                      </Text>
                      {!n.isRead && (
                        <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: '#F97316', marginLeft: 8 }} />
                      )}
                    </View>
                    <Text style={{ fontSize: 12, color: '#64748B', lineHeight: 18, marginBottom: 8 }}>
                      {n.message}
                    </Text>
